...
/// Invariant enforcement (fee_micro_pct <= 1_000_000) is the caller's responsibility;
...
module sweefi::math {
    ...
    // validate fee_micro_pct <= 1_000_000 before calling. The confusing error code
    ...
    ///
    /// `fee_micro_pct`: fee rate where 1_000_000 = 100%. See FEE_DENOMINATOR for common values.
    ///
    /// PRECONDITION: callers MUST validate `fee_micro_pct <= 1_000_000` before calling.
    ...
    ///   result = amount * fee_micro_pct / 1_000_000 ≤ amount (since fee_micro_pct ≤ 1_000_000)
    ...
    public fun calculate_fee(amount: u64, fee_micro_pct: u64): u64 {
        // Not just an optimization: fee_micro_pct == 0 must ALWAYS return 0 regardless
        ...
        // Defense-in-depth: technically redundant given fee_micro_pct <= 1_000_000 precondition,
    ...
    ///
    /// Returns max(calculated_fee, min_fee) when fee_micro_pct > 0.
    /// Use when the facilitator requires at least `min_fee` base units per tx.
    ///
    /// The `fee_micro_pct > 0` guard is intentional: a zero fee_micro_pct means "free transaction"
    /// — we must not apply a minimum floor to a deliberately free payment.
    public fun calculate_fee_min(amount: u64, fee_micro_pct: u64, min_fee: u64): u64 {
        let fee = calculate_fee(amount, fee_micro_pct);
        if (fee_micro_pct > 0 && fee < min_fee) {
            min_fee
        } else {
            fee
        }
